document.addEventListener("DOMContentLoaded", () => {
  ...
  if (form && status) {
    form.addEventListener("submit", (e) => {
      e.preventDefault();
      status.textContent = "";
      status.className = "form-status-j92m";
      ...
      if (!name || !email || !message) {
        ...
        status.classList.add("is-error-n83b");
        return;
      }
      ...
      status.classList.add("is-success-r30q");
  ...
  if (scrollBtn) {
    window.addEventListener("scroll", () => {
      if (window.scrollY > 300) scrollBtn.classList.add("is-visible-p44m");
      else scrollBtn.classList.remove("is-visible-p44m");
    });
